...
fine with an ordinary browser User-Agent. Going through the API gives you the
*source wikitext* (the {{Ship}} infobox template, the 'Known vessels' list,
citation templates) instead of rendered HTML you'd have to re-parse.

Stdlib only -- runs the same on Windows and Linux with no pip installs.
(Sibling copy of the one in ../../honorverse-system-scribe/scripts/.)

Usage:
    python fetch_wikitext.py "Reliant class"
    python fetch_wikitext.py "Sultan class" --save _raw/sultan-class.wiki
"""
...
def main() -> None:
    ...
    ap.add_argument("title", help="exact page title, e.g. 'Reliant class'")
    ap.add_argument(
        "--save",
        metavar="PATH",
