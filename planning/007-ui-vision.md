...
| **026** | **engine: `POST /plan`** — relocate the route-finder into `hvsim` so the API can plan over HTTP (see the Controller plan below); return a filed-route preview (not filed); decide the min-layover filing policy. | engine |
