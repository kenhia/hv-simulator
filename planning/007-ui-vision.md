...
| **029** ✅ | **Map polish** — adaptive **scale bar** (ly↔AU↔light-min↔light-sec↔km, #71), wormhole-**terminus markers** in non-host systems (⇄ → the junction's queue panel, #74), **queued ships drawn at the nexus** not the star (#75), **label click-targets** (#70). All UI; shared `scalebar.ts` / `overlays.ts` helpers. | ui |
...
| 032 | Repeating routes — ships live on auto-cycling itineraries (#59); the map bustles unattended. | ui+engine |
